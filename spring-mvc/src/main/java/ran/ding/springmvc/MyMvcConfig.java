...
public class MyMvcConfig extends WebMvcConfigurerAdapter {
	 ...
	 * HttpMessageConverter 有 两 个 方 法： configureMessageConverters： 重 载 会 覆 盖 掉
	 * Spring MVC 默 认 注 册 的 多 个 HttpMessageConverter。 extendMessageConverters： 仅 添
	 * 加一 个 自 定 义 的 HttpMessageConverter， 不 覆 盖 默 认 注 册 的 HttpMessageConverter。
	 */
	@Bean
	...
	public void extendMessageConverters(List<HttpMessageConverter<?>> converters) {
		 ...
		 * iterator.next(); if (httpMessageConverter instanceof
		 * StringHttpMessageConverter) { iterator.remove(); } }
